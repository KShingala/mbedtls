...
void * mt_mem_alloc(uint32_t size)
{
    printf("MT_AL_REALLOC %d\n", size);
    return malloc(size);
}

void * mt_mem_realloc(void * mem, uint32_t size)
{
    printf("MT_AL_ALLOC %d\n", size);
    return realloc(mem, size);
}

void * mt_mem_calloc(uint32_t size, uint32_t count)
{
    printf("MT_AL_CALLOC %d\n", size * count);
...
{
  return mt_mem_calloc(1, sizeof(mt_al_t));
}
...
mt_error_t mt_al_add(mt_al_t *mt_al, const mt_hash_t hash)
{
  ...
  if (mt_al->elems == 0) {
    // Add first element
    mt_al->store = mt_mem_alloc(HASH_LENGTH);
    if (!mt_al->store) {
    ...
    uint8_t *tmp = mt_mem_realloc(mt_al->store, alloc);
    if (!tmp) {
      return MT_ERR_OUT_Of_MEMORY;
    }
