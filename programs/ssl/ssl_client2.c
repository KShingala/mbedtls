...
static mt_t *sevrer_merkle_tree;

static uint8_t server_raw_key_hash[SERVER_COUNT][HASH_LENGTH] =
{
  //  0,    1,    2,    3,    4,    5,    6,   7,
  // 1_server
  {
...
void server_tree_create(void)
{
    sevrer_merkle_tree = mt_create();
    mt_print(sevrer_merkle_tree);

    for (uint32_t i = 0; i < SERVER_COUNT; ++i)
    {
        mt_add(sevrer_merkle_tree, server_raw_key_hash[i], HASH_LENGTH);
    }
...
int server_tree_search(const unsigned char * raw_key, uint32_t keylen)
{
    int ret = 0;
    int index = 0;
    unsigned char hash[32];

    ret = mbedtls_sha256_ret(raw_key, keylen, hash, 0);
    buffer_print(hash, HASH_LENGTH);

    mbedtls_printf("Searching in server tree, number of elements = %d\n", sevrer_merkle_tree->elems);

    if (ret == 0)
    {
        for (index ; index < sevrer_merkle_tree->elems; index ++)
        {
            if (mt_verify(sevrer_merkle_tree, hash, HASH_LENGTH, index) == MT_SUCCESS)
            {
                mbedtls_printf("Found, node %d\n", index);
                return 0;
            }
        }
        mbedtls_printf("Search failed\n");
        return -1;
    }
...
int my_verify_raw(void *data, const unsigned char * raw_key, int length, int depth, uint32_t * flags)
{
    ...
    ret = server_tree_search(raw_key, length);
